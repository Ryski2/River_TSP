% Math 381 Project 1
% Variation 3: Maximimum river constraint
clc; clear; close all
...
A = 5; % Maximum number of rivers

% total number of rivers
N = length(river_length);

% K has an upper bound of N since we visit every river at most once.
% In this variation, however, we will go on at most A rivers where A < N.
K = A;  % max number of steps

B = 2 * 8; % max number of hours

% Creates file where output conditions will be printed and stored
file = fopen('project1_var3.lp', 'wt');

% Set optimization function - maximizing the amount of time on each river.
fprintf(file, 'max: ');
